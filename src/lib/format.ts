...
export function mensajeError(raw: string | undefined): string {
  ...
  if (m.includes('SOLO_REVENDEDOR'))    return 'Tu cuenta todavía no está habilitada como revendedor.'
  if (m.includes('PVP_INVALIDO'))       return 'El PVP debe ser un valor válido mayor que cero.'
  if (m.includes('PIN_NO_PERTENECE'))   return 'Ese pin no pertenece a una de tus compras.'
